...
console.log(✅ [RCON SENT]${command} | Response: latex
{response}`); await rcon.end(); return response; // Return the actual response string } catch (error) { console.warn(`⚠️ [RCON ATTEMPT 
...
try {
const response = await axios.get(https://discord.com/api/v10/channels/${channelId}/messages, { headers: { Authorization:Bot ${DISCORD_BOT_TOKEN} },
params: { limit: 15 }
});
...
const response = await axios.get(https://discord.com/api/v10/guilds/${guildId}/members/latex
{userId}`, { headers: { Authorization: `Bot 
...
if (mcUuid) {
try {
const profileRes = await axios.get(https://sessionserver.mojang.com/session/minecraft/profile/${mcUuid}, { timeout: 3000 }); if (profileRes.data && profileRes.data.name && profileRes.data.name !== mcUsername) { console.log(🔄 Minecraft username change detected for  {mcUsername} -> ${profileRes.data.name});
...
avatar: userData.avatar ? https://cdn.discordapp.com/avatars/${userData.id}/ {userData.discriminator % 5}.png`,
...
$or: [ { minecraftUuid }, { minecraftUsername: new RegExp(^ , 'i') } ] }); } else { existingLink = await MinecraftLink.findOne({ minecraftUsername: new RegExp(^ `, 'i') });
}
...
try {
await axios.post(
https://discord.com/api/v10/channels/${WHITELIST_NOTIFY_CHANNEL}/messages, { content:<@${app.discordId}&gt; You have been whitelisted!
},
{
headers: {
Authorization: Bot ${DISCORD_BOT_TOKEN}, 'Content-Type': 'application/json' } } ); console.log(✅ [Discord] Notification sent to <@${app.discordId}>);
} catch (err) {
...
// Send RCON Command
await sendRconCommand(whitelist remove ${app.minecraftUsername}); await sendRconCommand(kick ${app.minecraftUsername} You have been removed from the whitelist.);
...
latex
or: [ { minecraftUsername: new RegExp(`^

{query}latex
`, 'i') }, { discordUsername: new RegExp(`^
...
let cmd = /give${link.minecraftUsername}  {link.minecraftUsername} ${item.name};
